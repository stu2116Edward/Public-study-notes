...
function watchVideo(frameObj, v_done) {
    ...
    setTimeout(() => {
        ...
        if (v) {
            ...
            // 播放
            v.play();
            console.log("%c正在播放第 " + (v_done + 1) + " 个视频", "font-size:18px");
            // 循环获取播放进度
...
console.log("%c 欢迎使用本脚本，此科目有 %c %d %c 个小节，当前为 %c 第 %d 小节 %c -chao", "color:#6dbcff", "color:red", window.unit, "color:#6dbcff", "color:red", window.unitCount, "font-size:8px");
main();
