...
### 3.常见的环境变量
- `PATH`：指定可执行文件的搜索路径
- `HOME`：当前用户的主目录(就是用户登录linux系统中时，默认的目录）
- `USER`：当前用户的用户名
- `SHELL`：当前用户使用的 Shell 类型，它的通常值是/bin/bash
- `LANG`：系统语言和字符集设置
- `PWD`：当前工作目录
- `HISTSIZE`：历史记录数
- `LOGNAME`：当前用户的登录名
...
- `PS1`：定义命令行提示符格式，对于 root 用户是 `#`，对于普通用户是 `$`
- `PS2`：定义多行命令的提示符的格式
...
5. **定义变量时的注意事项**：定义变量时，**变量名和等号之间不能有空格**
```bash
name="edward"  # 正确
...
```
***使用export定义路径的时候可加双引号也可不加***

**注意事项**：  
- 生效时间：立即生效  
- 生效期限：当前终端有效，窗口关闭后无效  
- 生效范围：仅对当前用户有效  
- 配置的环境变量中`不要忘了加上原来的配置`，即$PATH部分，避免覆盖原来配置（这里的意思是把路径写完整包括原有路径）
...
- `.profile`：如果 `.bash_profile` 和 `.bash_login` 都不存在，则会尝试加载此文件。这是一个`通用的配置文件`，适用于多种类型的 shell  
- `.bashrc`：当前用户的bash shell配置文件，这是在`每次打开新的终端窗口时加载的文件`。它包含了用户定义的别名、函数、自定义环境变量等  
...
### 7.删除环境变量
- 删除`永久性`的环境变量需要`进入到配置文件中删除相应的命令`  
    - 系统环境变量
    ```bash
    vim /etc/profile
    ```
...
- 删除`临时性`的环境变量需要使用到`unset （环境变量名称）`命令，如:
    ```bash
    unset PATH
    ```
