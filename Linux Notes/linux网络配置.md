...
虚拟机与外部主机处于同一网络段，相当于直接连接到物理网络的一个独立设备。  
虚拟机既能与局域网内的其他主机通信，也能访问外部网络。  
由于与局域网内的其他设备共享IP地址范围，容易发生IP地址冲突。  

### 仅主机模式（Host-Only Mode）
默认使用VMnet1，并提供DHCP服务。  
通常情况下，虚拟机无法访问外部网络，只能与宿主机（物理主机）进行通信。  
虚拟机可以访问宿主机，但无法访问局域网内的其他设备。  

### NAT模式（Network Address Translation Mode）
默认使用VMnet8，并提供DHCP服务。  
虚拟机可以与宿主机互相访问，并且能够访问外部网络。  
虚拟机无法访问局域网内的其他机器。  
不会与局域网内的其他IP地址发生冲突，因为虚拟机的IP地址由宿主机的NAT服务进行转换。  
