...
### 9.scp命令简介
`scp`（Secure Copy Protocol）是基于SSH协议的安全文件拷贝命令，用于在本地和远程主机之间复制文件和目录。
...
```
在这个命令中：
- `user@remote_host:/home/user/example.txt` 是远程主机上的文件路径
- `.` 表示当前目录

3. 递归复制目录
```bash
scp -r 本地目录 用户名@远程主机地址:/目标路径
```
如果你想**复制整个目录及其内容**，可以使用 `-r` 选项。例如，将本地的 my_directory 目录复制到远程主机的 /home/user/ 目录下：
```bash
scp -r mydir/ user@192.168.1.100:/home/user/
```

4. 指定端口号
如果远程主机的SSH端口不是默认的22（例如 2222），可以使用`-P`选项：
```bash
scp -P 2222 文件名 用户名@远程主机地址:/目标路径
```

5. 保留文件属性
如果你想在复制文件时保留文件的修改时间、访问时间和权限，可以使用 `-p` 选项：
```bash
scp -p 文件名 用户名@远程主机地址:/目标路径
```

6. 启用压缩
对于大文件或网络较慢的情况，可以使用 -C 选项启用压缩，以加快传输速度：
```bash
scp -C 文件名 用户名@远程主机地址:/目标路径
```
假设你有一个未压缩的大文件large_file.mp4，你可以使用以下命令：
```bash
scp -C large_file.mp4 user@host:/home/user/
```
在scp命令中使用-C选项时，它并不是对文件本身进行压缩，而是在传输过程中对数据流进行压缩

**注意事项**
1. 确保你有权限访问源文件和目标路径。
2. 如果远程主机需要SSH密钥认证，请确保密钥已配置好。
3. 使用`-v`选项可以帮助调试问题。
...
1. 作用  
管道符 `|` 是Linux中一个非常强大的命令操作符，它允许你将多个命令串联起来，使得前一个命令的输出成为后一个命令的输入。这样可以实现数据的连续处理，提高命令的灵活性和效率。  

2. 基本用法  
管道命令是**从左往右**执行的:  
- **格式**：`command1 | command2 | command3`
- **解释**：`command1` 的输出作为 `command2` 的输入，`command2` 的输出作为 `command3` 的输入，依此类推。
...
- **解释**：
  ...
  - `less`：分页显示输出内容，方便查看。
  - 使用管道符后，`ls -l` 的输出会被 `less` 逐页显示，按回车键或滚动鼠标滚轮可以查看下一页，按 `Q` 键退出。
...
- **解释**：
  - `echo "2+4"`：输出文本 "2+4"。
  - `bc`：一个计算器程序，可以对输入的表达式进行计算。
  - 使用管道符后，`echo` 的输出 "2+4" 会被 `bc` 作为输入并计算结果，输出为 `6`。
...
```
- **解释**：
  - `cat hello.sh`：显示文件 `hello.sh` 的内容。
  - `sort`：对输入的文本进行排序。
  - 使用管道符后，`cat` 的输出会被 `sort` 排序。
...
- **解释**：
  ...
  - `sort`：对输入的文本进行排序。
  - `uniq`：去除排序后的重复行。
  - 使用管道符后，`cat` 的输出先被 `sort` 排序，然后 `uniq` 去除重复行。
