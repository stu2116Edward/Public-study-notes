...
```
以下是前台模式和后台模式（使用 -d 选项）的主要区别：  
前台模式：  
容器的输出会直接显示在您的终端上  
您可以通过 `Ctrl+C` 来停止容器  
如果您`关闭终端或断开连接，容器通常会被停止`  

后台模式：  
容器会在后台运行，不会占用当前的命令行会话  
容器的输出不会显示在您的终端上，除非您使用 docker logs 命令来查看  
即使您`关闭终端或断开连接，容器也会继续运行`  
...
## Docker 容器数据卷
1. 数据卷介绍  
数据卷是 Docker 中用于数据持久化的机制。它允许您将容器内的数据保存在宿主机上，即使容器被删除，数据也不会丢失  
...
docker volume prune
```
这个命令会删除所有没有被任何容器引用的数据卷，释放空间  

7. 使用数据卷运行容器
```bash
docker run -d -v <卷名称或路径>:<容器内路径> <镜像名称>
```
- `<卷名称或路径>` 是您要挂载的数据卷名称或宿主机上的路径。
- `<容器内路径>` 是容器内的目标路径。
- `<镜像名称>` 是您要运行的 Docker 镜像名称。
...
- **具名挂载**：明确指定数据卷的名称
 ...
 ```
- **匿名挂载**：不指定名称，Docker 会自动创建一个匿名数据卷
 ```bash
 docker run -d -v <容器内路径> <镜像名称>
 ```

9. 从其他容器挂载数据卷
```bash
docker run -d --volumes-from <源容器名称> <目标容器镜像名称>
```
- `<源容器名称>` 是已有数据卷的容器名称。
- `<目标容器镜像名称>` 是新容器的镜像名称。
...
10. 设置数据卷为只读
```bash
docker run -d -v <卷名称或路径>:<容器内路径>:ro <镜像名称>
```
- `:ro` 表示挂载为只读模式。
